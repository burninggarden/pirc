...
class UserDetails {
	...
	isPreregistrationStub() {
		return this.is_preregistration_stub;
	}

	setIsPreregistrationStub() {
		this.is_preregistration_stub = true;
		return this;
	}
	...
	addChannelUserModeForChannelName(channel_user_mode, channel_name) {
		if (!this.hasChannelName(channel_name)) {
			throw new NotOnChannelError(channel_name);
		}
...
UserDetails.createPreregistrationStub = function createPreregistrationStub() {
	return (new UserDetails()).setIsPreregistrationStub();
};
