...
var
	Enum_ChannelUserPrefixes   = req('/lib/enum/channel-user-prefixes'),
	Enum_ChannelEnum_UserModes = req('/lib/enum/channel-user-modes'),
	Enum_UserModes             = req('/lib/enum/user-modes'),
	Enum_ModeActions           = req('/lib/enum/mode-actions'),

	ChannelDetails             = req('/lib/channel-details'),
	UserIdParser               = req('/lib/parser/user-id');

var
	NicknameValidator        = req('/lib/validator/nickname'),
	HostnameValidator        = req('/lib/validator/hostname'),
	UsernameValidator        = req('/lib/validator/username'),
	RealnameValidator        = req('/lib/validator/realname'),
	UserModeValidator        = req('/lib/validator/user-mode'),
	TimestampValidator       = req('/lib/validator/timestamp'),
...
class UserDetails {
	...
	handleModeMessage(message) {
		...
		this.dispatchModeChangeCallbacks(null);
	}
	...
	handleUnawayMessage(message) {
		this.removeMode(Enum_UserModes.AWAY);
		this.dispatchUnawayCallbacks(null);
	}

	handleNowAwayMessage(message) {
		this.addMode(Enum_UserModes.AWAY);
		this.dispatchAwayCallbacks(null);
	}
	...
	addModeChangeCallback(callback) {
		add(callback).to(this.getModeChangeCallbacks());
	}
	...
	dispatchModeChangeCallbacks(error) {
		var mode_change_callbacks = this.resetModeChangeCallbacks();

		while (mode_change_callbacks.length) {
			mode_change_callbacks.shift()(error);
		}
	}
	...
	addAwayCallback(callback) {
		add(callback).to(this.getAwayCallbacks());
	}

	getAwayCallbacks() {
		if (!this.away_callbacks) {
			this.resetAwayCallbacks();
		}

		return this.away_callbacks;
	}

	resetAwayCallbacks() {
		var prior_callbacks = this.away_callbacks;
	...
	dispatchAwayCallbacks(error) {
		var callbacks = this.resetAwayCallbacks();
	...
	addUnawayCallback(callback) {
		add(callback).to(this.getUnawayCallbacks());
	}

	getUnawayCallbacks() {
		if (!this.unaway_callbacks) {
			this.resetUnawayCallbacks();
		}

		return this.unaway_callbacks;
	}

	resetUnawayCallbacks() {
		var prior_callbacks = this.unaway_callbacks;

		this.unaway_callbacks = [ ];

		return prior_callbacks || [ ];
	}

	dispatchUnawayCallbacks(error) {
		var callbacks = this.resetUnawayCallbacks();
