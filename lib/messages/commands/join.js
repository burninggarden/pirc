...
class JoinMessage extends CommandMessage {

	setChannelKey(channel_name, channel_key) {
		var channel_details = this.getChannelTargetByName(channel_name);

		channel_details.setKey(channel_key);
	...
	getKeyForChannelName(channel_name) {
		return this.getChannelTargetByName(channel_name).getKey();
	}
...
extend(JoinMessage.prototype, {
	command: Commands.JOIN,
	abnf:    '"0" / ( <channel-name> *("," <channel-name>) [<channel-key> *("," <channel-key>)] )'
});
