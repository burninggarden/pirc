	...
	handleParameterParsingError(error) {
		if (error instanceof Heket.InputTooLongError) {
			throw new InvalidNicknameError(
				error.getValue(),
				ErrorReasons.OVER_MAXIMUM_LENGTH
			);
		}
		...
		if (error instanceof Heket.InputTooShortError) {
			throw new InvalidNicknameError(
				error.getValue(),
				ErrorReasons.UNDER_MINIMUM_LENGTH
			);
		}

		if (error instanceof Heket.InvalidRuleValueError) {
			// The following parsing error (where we encounter a CR but were
			// still expecting to read the nickname) happens when the user
			// fails to send any nickname at all.
			if (error.getRuleName() === 'CR') {
				throw new InvalidNicknameError(null, ErrorReasons.OMITTED);
			}
		}
