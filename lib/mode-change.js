...
class ModeChange {
	...
	getNickname() {
		if (this.hasUserTarget()) {
			return this.getTarget().getNickname();
		}

		throw new Error('Cannot access nickname for non-user mode target');
	}
	...
	getChannelName() {
		...
		throw new Error(
			'Cannot access channel name for non-channel mode target'
		);
	}
