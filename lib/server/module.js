...
class Server_Module {
	 ...
	 */
	getClients() {
		return this.getServer().getClientConnections();
	}

	/**
	 * @returns {Server_Connection_Server[]}
	 */
	getServers() {
		return this.getServer().getServerConnections();
	}

	getChannels() {
		var channels_module = this.getModuleByType(Enum_ModuleTypes.CHANNELS);

		return channels_module.getChannels();
	}
