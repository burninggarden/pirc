...
class InboundConnection extends Connection {
	...
	handleInboundMessageParsingError(error) {
		...
		switch (error.getCode()) {
			case ErrorCodes.INVALID_CHANNEL:
				return this.handleInvalidChannelParsingError(error);

			case ErrorCodes.INVALID_COMMAND:
				return this.handleInvalidCommandParsingError(error);

			case ErrorCodes.INVALID_NICK:
				return this.handleInvalidNickParsingError(error);

			case ErrorCodes.INVALID_TARGET:
				return this.handleInvalidTargetParsingError(error);

			case ErrorCodes.NOT_ENOUGH_PARAMETERS:
				return this.handleNotEnoughParametersError(error);

			/*
			default:
				let message = new NoticeMessage();
				message.setBody(error.getMessage());
				message.addTarget(ServiceDetails.fromName('Core'));
	...
	handleInvalidCommandParsingError(error) {
		var message = new UnknownCommandMessage();

		message.setCommand(error.getCommand());
