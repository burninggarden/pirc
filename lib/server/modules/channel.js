...
class ChannelModule extends Module {
	...
	createChannelForChannelName(channel_name) {
		...
		channel.setLocalServerDetails(server_details);
	...
	destroyChannel(channel) {
		...
		this.getLocalServerDetails().decrementChannelCount();
	}
	...
	sendNamesToClientForClientsNotInVisibleChannels(client) {
		...
		(new Channel('*'))
			.setLocalServerDetails(this.getLocalServerDetails())
	...
	handleClientTopicMessage(client, message) {
		...
		if (!channel_targets.length) {
			return this.sendNeedMoreParametersMessageToClientForCommand(
				client,
				Commands.TOPIC
			);
		}
