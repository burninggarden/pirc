...
var
	...
	StatsDLineMessage    = req('/lib/messages/replies/statsd-line'),
...
class ServerInfoModule extends Module {
	...
	hasMotd() {
		return this.getLocalServerDetails().hasMotd();
	}

	getMotd() {
		return this.getLocalServerDetails().getMotd();
	}

	setMotd(motd) {
		this.getLocalServerDetails().setMotd(motd);
	}
	...
	sendISupportMessagesToClient(client) {
		var messages = MessageBuilder.buildMultipleMessages(
			this.getLocalServerDetails().serializeSettings(),
	...
	sendStatsDLineMessageToClient(client) {
		this.sendMessageToClient(new StatsDLineMessage(), client);
	}
