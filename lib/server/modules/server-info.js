...
class ServerInfoModule extends Module {
	...
	getServerCount() {
		return this.getServerDetails().getGlobalServerCount();
	}
	...
	getUnknownUserCount() {
		return this.getServerDetails().getUnknownUserCount();
	}
	...
	sendMyInfoMessageToClient(client) {
		...
		message.setServerName(server_details.getHostname());
	...
	sendLUserClientMessageToClient(client) {
		var message = (new LUserClientMessage())
			...
			.setServiceCount(this.getServiceCount())
