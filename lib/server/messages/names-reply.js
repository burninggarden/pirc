...
class ServerNamesReplyMessage extends ServerMessage {
	 ...
	 */
	getPrivacySignifier() {
		if (this.channelIsPublic()) {
			return '=';
		} else if (this.channelIsSecret()) {
			return '@';
		} else {
			return '*';
		}
	...
	applyPrivacySignifier(signifier) {
		switch (signifier) {
			case '=':
				return this.setChannelIsPublic(true);

			case '@':
				return this.this.setChannelIsSecret(true);
	...
	setChannelIsPublic(state) {
		this.channel_is_public = state;
		return this;
	}

	channelIsPublic() {
		return this.channel_is_public;
	}

	setChannelIsSecret(state) {
		this.channel_is_secret = state;
		return this;
	}

	channelIsSecret() {
		return this.channel_is_secret;
	}

	serializeParams() {
		var
			targets           = this.serializeTargets(),
			privacy_signifier = this.getPrivacySignifier(),
	...
	applyParsedParams(middle_params, trailing_param) {
		...
		this.applyPrivacySignifier(middle_params[1]);

		var channel_name = middle_params[2];

		this.setSourceChannelName(channel_name);
