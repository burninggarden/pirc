...
class ServerChannel extends Channel {
	...
	getClientNames() {
		return this.clients.map(function map(client) {
			return client.getClientDetails().getNick();
		});
	}
	...
	createNamesReplyMessageForClient(client) {
		...
		message.addTarget(client.getClientDetails());
	...
	dispatchJoinMessagesForClient(joining_client) {
		this.clients.forEach(function each(client) {
			...
			message.setClientDetails(joining_client.getClientDetails());
			message.setChannelDetails(this.getChannelDetails());
	...
	dispatchPartMessagesForClient(parting_client) {
		this.clients.forEach(function each(client) {
			if (client === parting_client) {
				// We don't have to send a message to the client
				// who's doing the parting.
				// return;
			}

			var message = new PartMessage();

			message.setClientDetails(parting_client.getClientDetails());
			message.setChannelDetails(this.getChannelDetails());
	...
	handleClientTopicMessage(client, message) {
		...
		this.setTopicAuthorNick(message.getClientDetails().getNick());
