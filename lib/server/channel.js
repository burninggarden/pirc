...
var
	...
	Message_Command_Njoin   = require('../message/command/njoin'),
...
class Server_Channel extends Channel {
	...
	createNjoinMessage() {
		return (new Message_Command_Njoin())
			.setChannelName(this.getName())
			.setOriginHostname(this.getHostname());
	}
	...
	sendNamesToServer(server) {
		...
		var messages = MessageBuilder.buildMultipleMessages(
			body,
			this.createNjoinMessage.bind(this)
		);

		messages.forEach(server.sendMessage, server);
	}
