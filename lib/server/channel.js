...
class ServerChannel extends Channel {
	...
	sendTopicToClient(client) {
		...
		topic_details_message.setAuthorNickname(this.getTopicAuthorNickname());
	...
	dispatchPartMessagesForClient(parting_client) {
		this.clients.forEach(function each(client) {
			...
			message.addChannelName(this.getChannelName());
	...
	handleClientTopicMessage(client, message) {
		...
		var user_id = message.getUserId();

		throw new Error('fix user id > nickname ambiguity');

		this.setTopicAuthorNickname(user_id);
