...
var
	...
	TopicMessage               = req('/lib/server/messages/topic'),
	...
	ChannelIsFullMessage       = req('/lib/server/messages/channel-is-full'),
	NoChannelModesMessage      = req('/lib/server/messages/no-channel-modes'),
	UnknownModeMessage         = req('/lib/server/messages/unknown-mode');
...
class ServerChannel extends Channel {
	...
	sendTopicToClient(client) {
		...
		topic_details_message.setAuthorIdentifier(this.topic_author_identifier);
	...
	handleClientTopicMessage(client, message) {
		...
		var user_identifier = message.getUserDetails().getIdentifier();

		this.setTopicAuthorIdentifier(user_identifier);

		this.setTopicTimestamp(getCurrentTimestamp());

		this.clients.forEach(function each(target_client) {
			var outbound_message = TopicMessage.fromInboundMessage(message);
	...
	handleClientModeMessage(client, message) {
		...
		var mode_changes = message.getModeChanges();

		mode_changes.forEach(function each(mode_change) {
			this.handleClientModeChange(client, mode_change);
		}, this);
	}
	...
	handleClientModeChange(client, mode_change) {
		...
		if (!has(ChannelModes, mode) && !has(ChannelUserModes, mode)) {
			return this.sendUnknownModeMessageToClientForModeChange(
				client,
				mode_change
			);
		}
