...
class UserService extends Service {
	...
	handleClientModeChange(client, mode_change) {
		...
		if (!server_details.hasUserMode(mode)) {
			return this.sendUModeUnknownFlagMessageToClient(mode, client);
		}

		try {
			mode_change.validate();
		} catch (error) {
			return this.handleClientModeChangeError(error);
		}

		if (!this.clientHasPermissionToSetMode(client, mode)) {
			return this.sendNoPrivilegesMessageToClientForMode(client, mode);
		}
		...
		switch (action) {
			case ModeActions.ADD:
				user_details.addMode(mode);
				break;

			case ModeActions.REMOVE:
				user_details.removeMode(mode);
				break;
	...
	sendNoPrivilegesMessageToClientForMode(client, mode) {
	...
	handleClientModeChangeError(error) {
		throw error;
	}
