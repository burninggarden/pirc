...
class Connection extends EventEmitter {
	...
	handleSocketConnect() {
		...
		this.emit(ConnectionEvents.CONNECTION_SUCCESS, this);
	}
	...
	handleSocketError(error) {
		...
		this.setIsConnecting(false);
		this.setIsDisconnecting(false);
		this.emit(ConnectionEvents.CONNECTION_FAILURE, this, error);
	}
	...
	handleSocketData(data) {
		this.emit(ConnectionEvents.CONNECTION_DATA, this, data);
	...
	handleSocketEnd() {
		...
		this.setIsConnecting(false);
		this.setIsDisconnecting(false);
		this.emit(ConnectionEvents.CONNECTION_END, this);
	}
	...
	sendRawMessage(raw_message) {
		...
		if (raw_message.slice(-2) !== Delimiters.CRLF) {
			raw_message += Delimiters.CRLF;
		}
	...
	write(message) {
		...
		this.emit(ConnectionEvents.OUTGOING_MESSAGE_START, this, message);
	...
	handleWriteComplete(error) {
		...
		this.emit(ConnectionEvents.OUTGOING_MESSAGE_END, this);
	}
