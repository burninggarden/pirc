...
var
	...
	Server_Module_Capabilities    = require('./server/module/capabilities'),
...
class Server {
	...
	initModules() {
		...
		this.initModule(Server_Module_Capabilities);
	...
	coupleToConnection(connection) {
		...
		connection.on(
			Enum_ConnectionEvents.INVALID_MESSAGE,
			this.handleConnectionInvalidMessageError.bind(this)
		);
	...
	upgradeConnectionToClientConnection(connection) {
		var client_connection = this.upgradeConnectionToConstructor(
			connection,
	...
	handleConnectionIncomingMessage(connection, message) {
		...
		if (connection.isClientConnection()) {
			modules = this.getModulesForClientCommandMessage(message);
		} else if (connection.isServerConnection()) {
			modules = this.getModulesForServerCommandMessage(message);
		}
	...
	handleConnectionRegistrationSuccess(connection) {
		...
		this.coupleModulesToConnection(connection);
	...
	coupleModulesToConnection(connection) {
		this.getModules().forEach(function each(module) {
	...
	/**
	 * This handler is invoked when a connection is unable to parse an inbound
	 * message..
	 *
	 * @param   {Server_Connection} connection
	 * @param   {error} error
	 * @returns {void}
	 */
	handleConnectionInvalidMessageError(connection, error) {
		if (!connection.isConnected()) {
			return;
		}

		var reply = Message.fromReply(Enum_Replies.ERR_UNKNOWNCOMMAND);

		reply.setAttemptedCommand(error.getRawMessage());
	 ...
	 * @returns {Server_Module[]}
	 */
	getModulesForClientCommandMessage(message) {
		...
		switch (command) {
			...
			case Enum_Commands.CAP:
				return [this.getModuleByType(Enum_ModuleTypes.CAPABILITIES)];
	...
	getModulesForServerCommandMessage(message) {
		var command = message.getCommand();
