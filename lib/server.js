...
class Server extends Net.Server {
	...
	constructor(parameters) {
		...
		if (parameters.authenticateUser !== undefined) {
			this.getModuleByType(ModuleTypes.AUTH).setAuthenticationCallback(
				parameters.authenticateUser
			);
		}

		if (parameters.port) {
			this.listen(parameters.port);
		}
	}
	...
	destroy(callback) {
		...
		this.client_connections.forEach(function each(client_connection) {
			chain.andCall(client_connection.disconnect).bind(client_connection);
		});

		this.server_connections.forEach(function each(server_connection) {
			chain.andCall(server_connection.disconnect).bind(server_connection);
		});

		this.pending_connections.forEach(function each(pending_connection) {
			chain.andCall(pending_connection.disconnect).bind(pending_connection);
		});
	...
	dispatchDestructionCallbacks(error) {
		this.getDestructionCallbacks().forEach(function each(callback) {
			callback(error);
		});
	}
