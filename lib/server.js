...
var
	ServerMessageParser = req('/lib/server/message-parser'),
	ClientMessageParser = req('/lib/client/message-parser');
...
class Server extends Net.Server {
	 ...
	 */
	instantiateMessages() {
		each(ReplyNumerics, function each(key, reply_numeric) {
			createReply(reply_numeric);
		});

		each(Commands, function each(key, command) {
			ClientMessageParser.createMessageForCommand(command);
			ServerMessageParser.createMessageForCommand(command);
		});
	}
	...
	sendMessageToClientConnectionViaReplyNumeric(
		client_connection,
		reply_numeric
	) {
		var reply = createReply(reply_numeric);

		client_connection.sendMessage(reply);
