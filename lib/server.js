...
class Server extends Net.Server {
	...
	initServices() {
		...
		this.addService(new MotdService());
	...
	handleClientConnectionEnd(client_connection) {
		...
		this.getServices().forEach(function each(service) {
			service.decoupleFromClient(client_connection);
		});
	}
	...
	registerClientConnection(client_connection) {
		function handler(error) {
			...
			this.handleClientConnectionRegistered(client_connection);
		}

		var auth_service = this.getServiceByType(ServiceTypes.AUTH);

		auth_service.registerClient(client_connection, handler.bind(this));
	}
	...
	handleClientConnectionRegistered(client_connection) {
		...
		this.getServices().forEach(function each(service) {
			service.coupleToClient(client_connection);
		});
	}
	...
	getServicesForCommandMessage(message) {
		...
		switch (command) {
			...
			case Commands.NAMES:
			case Commands.TOPIC:
				return [this.getServiceByType(ServiceTypes.CHANNELS)];

			case Commands.PING:
			case Commands.PONG:
				return [this.getServiceByType(ServiceTypes.PINGS)];

			case Commands.USER:
				return [this.getServiceByType(ServiceTypes.USERS)];

			case Commands.NICK:
				return [this.getServiceByType(ServiceTypes.NICKS)];
	...
	getServicesForPrivateMessage(message) {
		...
		if (message.hasChannelTarget()) {
			services.push(this.getServiceByType(ServiceTypes.CHANNELS));
		}

		if (message.hasUserTarget()) {
			services.push(this.getServiceByType(ServiceTypes.PMS));
		}
	...
	getServices() {
		return this.getServiceRegistry().getEntries();
	}
