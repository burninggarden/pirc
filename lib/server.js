...
class Server {
	...
	upgradePendingConnectionToClientConnection(pending_connection) {
		...
		client_connection.setLogIncomingMessages(this.shouldLogIncomingMessages());
		client_connection.setLogOutgoingMessages(this.shouldLogOutgoingMessages());

		this.coupleToClientConnection(client_connection);

		pending_connection.getQueuedMessages().forEach(function each(message) {
			client_connection.handleIncomingMessageSafe(message);
		});
	...
	upgradePendingConnectionToServerConnection(pending_connection) {
		...
		pending_connection.getQueuedMessages().forEach(function each(message) {
			server_connection.handleIncomingMessageSafe(message);
		});
	...
	decoupleFromClientConnection(client_connection) {
	...
	handleClientConnectionOutgoingMessageStart(client_connection, message) {
		this.handleConnectionOutgoingMessageStart(client_connection, message);
	}
	...
	handleClientConnectionOutgoingMessageEnd(client_connection, message) {
		this.handleConnectionOutgoingMessageEnd(client_connection, message);
	...
	/**
	 * @param   {Server_Connection_Server} server_connection
	 * @returns {void}
	 */
	coupleToServerConnection(server_connection) {
		...
		server_connection.on(
			Enum_ConnectionEvents.INCOMING_MESSAGE,
			this.handleServerConnectionIncomingMessage.bind(this)
		);

		server_connection.on(
			Enum_ConnectionEvents.OUTGOING_MESSAGE_START,
			this.handleServerConnectionOutgoingMessageStart.bind(this)
		);

		server_connection.on(
			Enum_ConnectionEvents.OUTGOING_MESSAGE_END,
			this.handleServerConnectionOutgoingMessageEnd.bind(this)
		);
	...
	handleServerConnectionIncomingMessage(server_connection, message) {
		if (message.hasImmediateResponse()) {
			server_connection.sendMessage(message.getImmediateResponse());
		...
		if (!server_connection.isRegistered()) {
			if (server_connection.canRegister()) {
				this.registerServerConnection(server_connection);
			}
		...
		modules.forEach(function each(module) {
			module.handleServerMessage(server_connection, message);
		});
	}
	 ...
	 * @returns {void}
	 */
	handleServerConnectionOutgoingMessageStart(server_connection, message) {
		this.handleConnectionOutgoingMessageStart(server_connection, message);
	}
	 ...
	 * @returns {void}
	 */
	handleServerConnectionOutgoingMessageEnd(server_connection, message) {
		this.handleConnectionOutgoingMessageEnd(server_connection,message);
	}
	...
	handleServerConnectionEnd(server_connection) {
		this.getModules().forEach(function each(module) {
			module.decoupleFromServer(server_connection);
		});
