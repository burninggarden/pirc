...
var
	PendingConnection = req('/lib/server/connection/pending'),
	ClientConnection  = req('/lib/server/connection/client'),
	ServerConnection  = req('/lib/server/connection/server');
...
var
	...
	ClientRegistry = req('/lib/server/registry/clients'),
	ModuleRegistry = req('/lib/server/registry/modules');


var
	AuthModule       = req('/lib/server/module/auth'),
	ChannelModule    = req('/lib/server/module/channels'),
	PMModule         = req('/lib/server/module/pms'),
	NickModule       = req('/lib/server/module/nicknames'),
	UserModule       = req('/lib/server/module/users'),
	OperatorModule   = req('/lib/server/module/operators'),
	PingModule       = req('/lib/server/module/ping'),
	ServerInfoModule = req('/lib/server/module/server-info');
...
class Server {
	...
	constructor(parameters) {
		...
		if (parameters.authenticateUser !== undefined) {
			let module = this.getModuleByType(Enum_ModuleTypes.AUTH);
	...
	getModulesForCommandMessage(message) {
		...
		switch (command) {
			...
			case Enum_Commands.PING:
				return [this.getModuleByType(Enum_ModuleTypes.PING)];
	...
	getModulesForPrivateMessage(message) {
		...
		if (message.hasUserMessageTargets()) {
			add(this.getModuleByType(Enum_ModuleTypes.PMS)).to(modules);
		}
