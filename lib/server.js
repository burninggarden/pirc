...
class Server extends Net.Server {
	...
	handleClientConnectionMessage(client_connection, message) {
		...
		modules.forEach(function each(module) {
			module.handleClientConnectionMessage(client_connection, message);
		});
	}

	handleClientConnectionEnd(client_connection) {
		this.getClientConnectionRegistry().removeClientConnection(
			client_connection
		);

		this.getModules().forEach(function each(module) {
			module.decoupleFromClientConnection(client_connection);
		});
	...
	registerClientConnection(client_connection) {
		...
		function advance() {
			...
			module.registerClientConnection(
				client_connection,
				handler.bind(this)
			);
		}
	...
	handleClientConnectionRegistrationSuccess(client_connection) {
		...
		this.getClientConnectionRegistry().addClientConnection(client_connection);

		this.getModules().forEach(function each(module) {
			module.coupleToClientConnection(client_connection);
		});
	}
