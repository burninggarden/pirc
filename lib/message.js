...
var
	...
	InvalidBnfError                      = req('/lib/errors/invalid-bnf'),
...
class Message {
	...
	validateParameterCount(middle_parameters, trailing_parameter) {
		if (middle_parameters.length < this.getMiddleParameterCount()) {
	...
	throwInvalidParametersError(error_reason) {
		throw new this.getInvalidParametersError()(
			this.getRawMessage(),
			error_reason
		);
	}

	getMiddleParameterCount() {
		if (this.middle_parameter_count === null) {
			this.middle_parameter_count = this.getMiddleParameterCountFromBnf();
		}

		return this.middle_parameter_count;
	}

	getMiddleParameterCountFromBnf() {
	}

	expectsTrailingParameter() {
		if (this.expects_trailing_parameter === null) {
			this.expects_trailing_parameter = this.getTrailingParameterExpectedFromBnf();
		}
	...
	deserializeParams(raw_params) {
		...
		this.validateParamsCount(middle_params, trailing_param);
