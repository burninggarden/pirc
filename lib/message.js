...
class Message {
	...
	getMessagePartForStructureItem(structure_item, index) {
		switch (structure_item) {
			...
			case StructureItems.USER_IDENTIFIER:
	...
	getMessagePartAtIndex(index) {
		throw new Error('implement getMessagePartAtIndex() in child classes');
	}
	...
	processMessagePartAsStructureItem(message_part, structure_item, index) {
		switch (structure_item) {
			...
			case StructureItems.USER_IDENTIFIER:
				return this.processMessagePartAsUserIdentifier(message_part);
	...
	processMessagePartAsUserIdentifier(message_part) {
		// Trim off any leading ":" at the head of the message part:
		if (message_part[0] === ':') {
			message_part = message_part.slice(1);
		}

		var match = message_part.match(Regexes.USER_IDENTIFIER);

		if (!match) {
			throw new InvalidUserIdentifierError(
				message_part,
		...
		try {
			this.setNick(match[1]);
		} catch (error) {
			return void this.handleNickError(error);
		}

		try {
			this.setUsername(match[2]);
		} catch (error) {
			return void this.handleUsernameError(error);
		}

		try {
			this.setHostname(match[3]);
		} catch (error) {
			return void this.handleHostnameError(error);
		}
	}
	...
	isAtCharacterLimit() {
		return this.getCharacterCount() >= CHARACTER_LIMIT;
	}
