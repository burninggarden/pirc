...
var
	MessageUnparser = req('/lib/unparser/message'),
	PrefixUnparser  = req('/lib/unparser/prefix'),
	PrefixParser    = req('/lib/parser/prefix'),
	IrcRules        = req('/lib/rule-list/irc');
...
class Message {
	...
	serialize() {
		...
		try {
			return MessageUnparser.unparse(handler);
		} catch (error) {
			throw error;
		}
	}
