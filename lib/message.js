...
var
	...
	InvalidABNFError                     = req('/lib/errors/invalid-abnf'),
...
class Message {
	...
	validateABNF() {
		if (!this.hasABNF()) {
			throw new InvalidABNFError(null, ErrorReasons.OMITTED);
		}

		try {
			this.buildParameterParser();
		} catch (error) {
			throw new InvalidABNFError(null, ErrorReasons.WRONG_FORMAT);
		}
	}
	...
	buildParameterParser() {
		var abnf = this.getABNF();

		if (abnf.indexOf('parameters = ') !== 0) {
			abnf = 'parameters = ' + abnf;
		}

		this.constructor.parser = Heket.parse(abnf);
	}
	...
	serialize() {
		...
		// Per the RFC, the parameters segment should begin with a space.
		if (parameters[0] !== ' ') {
