...
class Message {
	...
	parsePrefix(prefix) {
		...
		if (hostname) {
			let server_details = ServerDetails.fromHostname(hostname);

			this.setOriginServerDetails(server_details);
		} else if (user_id) {
			let user_details = UserDetails.fromUserId(user_id);

			this.setOriginUserDetails(user_details);
		} else {
			throw new Error('wat');
		}
	}
	...
	getValuesForPrefix() {
		if (this.hasOriginUser()) {
			return this.getValuesForUserPrefix();
		} else if (this.hasOriginServer()) {
	...
	getValuesForServerPrefix() {
		return {
			hostname: this.getOriginServerHostname()
		};
	}
	...
	hasOriginUser() {
		return this.getOriginUserDetails() !== null;
	}

	getOriginUserDetails() {
		return this.origin_user_details;
	}

	setOriginUserDetails(user_details) {
		this.origin_user_details = user_details;
		return this;
	}

	getOriginUserId() {
		return this.getOriginUserDetails().getIdentifier();
	}
