...
class Client extends EventEmitter {
	...
	constructor(parameters) {
		...
		if (parameters.log_inbound_messsages) {
	...
	connectToServer(parameters, callback) {
		...
		function handleConnected(error) {
			...
			function handleRegistrationSuccess() {
				server_connection.removeListener(
					ServerConnectionEvents.REGISTRATION_FAILURE,
			...
			function handleRegistrationFailure(error) {
				server_connection.removeListener(
					ServerConnectionEvents.REGISTRATION_SUCCESS,
		...
		try {
			...
			return void server_connection.connect(handleConnected.bind(this));
		} catch (error) {
