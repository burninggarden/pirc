...
class ServerConnection extends Connection {
	...
	handleWelcomeMessage(message) {
		this.emit(ServerConnectionEvents.REGISTRATION_SUCCESS);
	}

	handlePasswordMismatchMessage(message) {
		let error = new Error('Invalid password');

		this.emit(ServerConnectionEvents.REGISTRATION_FAILURE, error);
	}
	...
	handleChannelOperatorPrivilegesNeededMessage(message) {
		this.getChannelForMessage(message).handleOperatorPrivilegesNeededMessage(message);
	}
