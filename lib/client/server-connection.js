...
var
	...
	deferOrThrow  = req('/lib/utility/defer-or-throw'),
	extend        = req('/lib/utility/extend'),
	has           = req('/lib/utility/has'),
	add           = req('/lib/utility/add'),
	remove        = req('/lib/utility/remove'),
	random        = req('/lib/utility/random'),
	isFunction    = req('/lib/utility/is-function'),
	isBoolean     = req('/lib/utility/is-boolean'),
	TextFormatter = req('/lib/utility/text-formatter');


var
	Connection                  = req('/lib/connection'),
	Client_Channel              = req('/lib/client/channel'),
	Client_Registry_UserDetails = req('/lib/client/registry/user-details'),
	...
	CallbackList                = req('/lib/callback-list');
...
var
	Message_Command_Away    = req('/lib/message/command/away'),
	Message_Command_Join    = req('/lib/message/command/join'),
	Message_Command_Part    = req('/lib/message/command/part'),
	Message_Command_Private = req('/lib/message/command/private'),
	Message_Command_Pass    = req('/lib/message/command/pass'),
	Message_Command_Nick    = req('/lib/message/command/nick'),
	Message_Command_Mode    = req('/lib/message/command/mode'),
	Message_Command_User    = req('/lib/message/command/user'),
	Message_Command_Topic   = req('/lib/message/command/topic'),
	Message_Command_Pong    = req('/lib/message/command/pong'),
	Message_Command_Whois   = req('/lib/message/command/whois'),
	Message_Command_Oper    = req('/lib/message/command/oper'),
	Message_Command_Quit    = req('/lib/message/command/quit'),
	Message_Command_Restart = req('/lib/message/command/restart');
...
class Client_ServerConnection extends Connection {
	...
	handleUnawayMessage(message) {
		this.getUserDetails().handleUnawayMessage(message);
	}

	handleNowAwayMessage(message) {
		this.getUserDetails().handleNowAwayMessage(message);
	}
	...
	/**
	 * @param   {lib/messages/replies/no-privileges} message
	 * @returns {void}
	 */
	handleNoPrivilegesMessage(message) {
		var error = new Error('You are not an IRC operator');

		this.getNoPrivilegesCallbacks().dispatch(error);
	}
	 ...
	 * the necessary parameters for creating the outbound socket.
	 *
	...
	setAwayMessage(away_message, callback) {
		...
		user_details.setAwayMessage(away_message);

		if (away_message) {
			user_details.addAwayCallback(callback);
		} else {
			user_details.addUnawayCallback(callback);
		}
	}
	...
	sendRestartMessage(callback) {
		...
		var
			disconnect_callbacks    = this.getDisconnectCallbacks(),
			no_privileges_callbacks = this.getNoPrivilegesCallbacks();

		function handleDisconnect() {
			no_privileges_callbacks.remove(handleNoPrivileges);

			callback(null);
		}

		function handleNoPrivileges(error) {
			disconnect_callbacks.remove(handleDisconnect);

			callback(error);
		}

		disconnect_callbacks.add(handleDisconnect);
		no_privileges_callbacks.add(handleNoPrivileges);
	}

	getNoPrivilegesCallbacks() {
		if (!this.no_privileges_callbacks) {
			this.no_privileges_callbacks = new CallbackList();
		}

		return this.no_privileges_callbacks;
	}
