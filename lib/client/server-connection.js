...
var
	AccessInterface           = req('/lib/client/server-connection/interfaces/access'),
	...
	ClientsInterface          = req('/lib/client/server-connection/interfaces/clients'),
	PingsInterface            = req('/lib/client/server-connection/interfaces/pings');
...
const INTERFACES = [
	...
	OutgoingMessagesInterface,
	IncomingMessagesInterface,
	ChannelsInterface,
	ClientsInterface,
	PingsInterface
];
...
class ServerConnection extends EventEmitter {
	...
	init() {
		this.callMethodAcrossInterfaces('init');
	}

	handleMessage(message) {
		this.callMethodAcrossInterfaces('handleMessage', [message]);
	}

	callMethodAcrossInterfaces(method_name, args) {
		INTERFACES.forEach(function each(current_interface) {
			let method = current_interface[method_name];
...
function applyInterface(interface_to_apply) {
	var
		keys_to_exclude          = Object.getOwnPropertyNames(ServerConnection.prototype),
		throw_error_on_collision = true;
...
INTERFACES.forEach(applyInterface);
