...
class ServerConnection extends Connection {
	...
	handleEndOfWhoisMessage(message) {
		...
		this.getUserDetailsRegistry().user_detail_records.forEach(function each(record) {
			if (record.getNick() === user_details.getNick()) {
				console.log('HIT: ' + record.getUniqueId());
				count++;
			}
		});
	...
	sendWhoisQueryForNick(nick, callback) {
		...
		if (callback) {
			let unique_id = user_details.getUniqueId();

			console.log('ADDING WHOIS CALLBACK FOR NICK: ' + user_details.getNick() + '(' + unique_id + ')');
