...
class ServerConnection extends Connection {
	...
	handleJoinMessage(message) {
		...
		var
			...
			channel      = this.getOrCreateChannelForChannelName(channel_name);
	...
	handleChannelIsFull(message) {
		...
		var error = new Error(`
			Unable to join channel ${channel_name}
	...
	handleLinkChannelMessage(message) {
		var
			...
			linked_channel_name = message.getLinkedChannelName(),
			desired_channel     = this.getChannelForChannelName(channel_name);

		var error = new Error(`
			Unable to join channel ${channel_name},
			forwarded to channel ${linked_channel_name}
		`);
	...
	handleUnknownModeMessage(message) {
		...
		channel.handleUnknownModeMessage(message);
	}

	handleInviteOnlyChannelMessage(message) {
		var
			channel_name    = message.getChannelName(),
			desired_channel = this.getOrCreateChannelForChannelName(channel_name);
	...
	getChannelForMessage(message) {
		return this.getChannelForChannelName(message.getChannelName());
	}
