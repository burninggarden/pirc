...
var
	...
	UserRegistry           = req('/lib/client/registries/users'),
...
class ServerConnection extends EventEmitter {
	...
	handleUserModeMessage(message) {
		this.getUserForMessage(message).handleModeMessage(message);
	}
	...
	handlePartMessage(message) {
		...
		delete this.channels_by_name[channel.getName()];
	...
	getUserForMessage(message) {
		return this.getUserRegistry().getUserForMessage(message);
	}
	...
	getUserDetailsForMessage(message) {
		return this.getUserRegistry().getUserDetailsForMessage(
			message
		);
	}
