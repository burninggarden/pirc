...
class Client_ServerConnection extends Connection {
	...
	handleReplyMessage(message) {
		...
		switch (reply) {
			...
			case Enum_Replies.ERR_NOSUCHSERVER:
				return this.handleNoSuchServerMessage(message);
	...
	handleNoSuchServerMessage(message) {
		var error = new Error('No such server: ' + message.getHostname());

		this.emit(Enum_ConnectionEvents.CONNECTION_ERROR, error);
	}
	 ...
	 * @returns {void}
	 */
	sendConnectMessage(hostname, port) {
