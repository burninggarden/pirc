...
class Client_ServerConnection extends Connection {
	...
	dispatchCallbackForIncomingMessage(incoming_message) {
		...
		if (incoming_message.isErrorMessage()) {
			error = incoming_message.toError();
		}

		var callback = sent_message.getCallback();
	...
	handleLUserChannelsMessage(message) {
		...
		server_details.setChannelCount(message.getRemoteChannelCount());
	}
	...
	handleNowAwayMessage(message) {
		this.getUserDetailsForMessage(message).addAwayMode();
	}
	...
	handleTopicMessage(message) {
		...
		channel.setTopic(message.getTopic());
	}
