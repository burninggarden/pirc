...
var
	ServerCapMessage      = req('/lib/server/messages/cap'),
	...
	ServerPongMessage     = req('/lib/server/messages/pong'),
	ServerPartMessage     = req('/lib/server/messages/part'),
	ServerPassMessage     = req('/lib/server/messages/pass'),
	...
	ServerUserMessage     = req('/lib/server/messages/user'),
	ServerWhoMessage      = req('/lib/server/messages/who'),
	ServerWhoisMessage    = req('/lib/server/messages/whois'),
	ServerWhowasMessage   = req('/lib/server/messages/whowas');
...
class ClientMessageParser extends MessageParser {
	...
	getConstructorForCommand(command) {
		switch (command) {
			...
			case Commands.WHO:
				return ServerWhoMessage;

			case Commands.WHOIS:
				return ServerWhoisMessage;

			case Commands.WHOWAS:
				return ServerWhowasMessage;
